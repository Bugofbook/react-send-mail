...
export function handleClick(link: HTMLAnchorElement): HTMLAnchorElement {
  if (link.href) {
    link.click()
  }
  return link
...
export function afterClick(link: HTMLAnchorElement): void {
  const windowFocus = () => {
...
export function bodyAppendLink(link: HTMLAnchorElement): HTMLAnchorElement {
